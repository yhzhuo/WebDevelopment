<?php
	...
	*/
	...
	$query = mysql_query("select need,exchange from swap_requestlist where uid=$uid limit 0,1");
...
*/
	...
	*/
	...
	if($numSecondary == 0) {   //search the both condition
		$query = str_replace("-selectStart-",$numPrimary,$query);
		$query = str_replace("-lines-","20",$query);
		//<debug> 
		//echo "modified query: ".$query."     ";
		//</debug>
	...
	$query = $sql;
	if(($numSecondary != 0 || $numResultRows < 20) && ($countn != 0 && $counte != 0)) {     //This part only do the single condition
		$query = str_replace(") && (",") xor (",$query);
		//echo "<p>2"."<p>";
		//echo "numSecondary: ".$numSecondary;
		
		if($numSecondary != 0) {     //all results are secondary
			$query = str_replace("-selectStart-",$numSecondary,$query);
			$query = str_replace("-lines-","20",$query);
			//echo "<hr />".$query."<hr />";  //
		...
		//echo "<hr />".$query."<hr />";
		//<debug> 
		//echo "modified query in numSecondary != 0 : ".$query."     ";
		//</debug>
