...
//This is a already robust version of sending email system
	if($_POST['pw'] != "111111") {
		echo '
		...
		function self_send($mailer, $autos) {
			...
			*/
			try {
				...
				echo "<br />Mailer Error: " . $e->getMessage();
				echo "<p>One exception happened, but the program should be excutable, just sleep for one hour</p>";
				sleep(3620);
			}
		}
		...
		while($f = mysql_fetch_array($q)) {   // && $debugCount < 2
			...
			*/
			if(count(split('@',$f[email])) != 1) {
				...
				*/
				if($makeDifference % 10000 == 0 && $makeDifference != 0) {
